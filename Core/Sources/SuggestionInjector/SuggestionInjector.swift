...
>>>>>>> 4a8ae39... Pre-release 0.22.73
        ...
        if let suggestionLineLimit {
            let allLines = toBeInserted
            toBeInserted = Array(toBeInserted.prefix(suggestionLineLimit))
            if suggestionLineLimit < allLines.count {
                // advance to the next line when accepting part of a multi-line suggestion
                toBeInserted.append(startOfLine(line: allLines[suggestionLineLimit], usingEnding: lineEnding))
            }
        }
...
>>>>>>> 4a8ae39... Pre-release 0.22.73
    ...
    func startOfLine(line: String, usingEnding lineEnding: String) -> String {
        return line.prefix(while: { $0.isWhitespace }).appending(lineEnding)
    }
